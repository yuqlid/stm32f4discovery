...
I2C_HandleTypeDef*	itgI2cHandle;

//Constructor assigns the i2c address for the gyro
ITG3200::ITG3200(I2C_HandleTypeDef* I2cHandle)
{
	itgI2cHandle = I2cHandle;
}
...
uint8_t ITG3200::read(uint8_t register_addr, uint8_t * value){
	uint8_t rxdata_buf;
	if(HAL_I2C_Mem_Read(itgI2cHandle, ITG_ADDR, register_addr, I2C_MEMADD_SIZE_8BIT, value, 1, 10)!=HAL_OK)Error_Handler;
//	*value = rxdata_buf;
//	return rxdata_buf;
}
...
uint8_t ITG3200::write(uint8_t register_addr, uint8_t value){

	if(HAL_I2C_Mem_Write(itgI2cHandle, ITG_ADDR, register_addr, I2C_MEMADD_SIZE_8BIT, &value, 1, 10)!=HAL_OK)Error_Handler();
...
bool ITG3200::isonline(){

	if(HAL_I2C_IsDeviceReady(itgI2cHandle, ITG_ADDR, 100, 100) != HAL_OK)return false;
	else return true;
